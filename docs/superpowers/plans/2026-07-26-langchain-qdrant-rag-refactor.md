...
## 후속 과제 (이번 계획 범위 밖, 별도 진행)

### 하이브리드 검색으로 지식카드 리콜 보완

**배경:** Task 6 리뷰에서 확인된 구조적 트레이드오프다. 리팩토링 이전 `retriever.py`는 QA 지식카드를 키워드 오버랩(`overlap_score = matches / len(query_words)`, 임계 `> 0.1`, `0.4 + overlap * 0.5`로 재스케일)으로 별도 검색했기 때문에, 임베딩 유사도가 낮아도 질의와 같은 어휘를 쓴 카드는 **반드시** 후보에 올랐다.

단일 Qdrant 컬렉션 + 단일 코사인 검색으로 통합하면서 이 보장이 사라졌다. 질의와 주제는 같지만 어휘가 다른 지식카드는 이제 `threshold=0.35`를 넘지 못해 누락될 수 있다.

**이번 계획에서 다루지 않는 이유:** 설계 명세서가 명시적으로 "하나의 Qdrant 컬렉션으로 통합"과 "키워드 오버랩 점수 계산 제거"를 요구했고, 사용자가 이를 인지한 상태에서 그대로 진행하기로 결정했다(2026-07-27). 구현 결함이 아니라 승인된 아키텍처 선택의 결과다.

**보완 방향:** Qdrant는 sparse 벡터를 기본 지원하므로, `langchain-qdrant`의 `RetrievalMode.HYBRID` + `FastEmbedSparse`(BM25 계열)로 밀집·희소 검색을 결합하면 어휘 일치 리콜을 되살릴 수 있다. `ingestion.py`의 적재 경로와 `vector_store.search()`, 그리고 관련 테스트를 함께 수정해야 하므로 독립 태스크로 다루는 편이 낫다. threshold를 낮추는 방식은 어휘 불일치 케이스를 해결하지 못하면서 무관한 근거만 늘리므로 대안이 아니다.
